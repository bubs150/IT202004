<?php
    ...
    if(isset($_REQUEST["email"])){
        ...
        if(is_empty_or_null($email)){
            $sql = "SELECT id, email, password from mt_users where username = '$guy'";
            echo "a";
        }else if(is_empty_or_null($guy)){
            $sql = "SELECT id, email, password from mt_users where email = '$email'";
            echo "b";
        }else{
            $sql = "SELECT id, email, password from mt_users where email = '$email' and username = '$guy'";
            echo "c";
        }
        
        $retVal = mysqli_query($db, $sql);//retrieves the attempted profile's login's creds for comparison to what was received
        if($retVal){
            ...
            if(password_verify($password, $result["password"])){
                ...
                die(header("Location: navigate.php"));
                //THIS IS GONNA TAKE US TO NAVIGATE. IT WAS SAYING HOME, BUT FUCK THAT NOISE WE DIDN'T NEED IT PLUS THAT SHIT WORKED SO THIS WILL TOO
